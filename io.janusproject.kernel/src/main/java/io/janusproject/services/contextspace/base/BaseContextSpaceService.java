...
@Singleton
@TwoStepConstruction
public class BaseContextSpaceService extends AbstractDependentService implements ContextSpaceService {
	 ...
	 */
	public BaseContextSpaceService() {
		//
	}
	...
	protected void fireContextCreated(AgentContext context) {
		...
		this.logger.info(BaseContextSpaceService.class, "CONTEXT_CREATED", context.getID()); //$NON-NLS-1$
	...
	protected void fireContextDestroyed(AgentContext context) {
		ContextRepositoryListener[] ilisteners = this.listeners.getListeners(ContextRepositoryListener.class);
		this.logger.info(BaseContextSpaceService.class, "CONTEXT_DESTROYED", context.getID()); //$NON-NLS-1$
		for (ContextRepositoryListener listener : ilisteners) {
