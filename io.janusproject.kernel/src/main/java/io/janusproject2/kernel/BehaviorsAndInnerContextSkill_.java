 ...
 */
class BehaviorsAndInnerContextSkill_ extends Skill implements Behaviors, InnerContextAccess {
	 ...
	 * @param agent
	 */
	public BehaviorsAndInnerContextSkill_(Agent agent) {
		super(agent);
	...
	void setInternalEventBus(AsyncSyncEventBus bus) {
		...
		if (this.logger != null)
			this.logger.debug("EVENT_BUS_CHANGE", getOwner().getID()); //$NON-NLS-1$
	}
	...
	private static class AgentEventListener implements EventListener {
		...
		private WeakReference<BehaviorsAndInnerContextSkill_> skill;

		private WeakReference<UUID> aid;

		@SuppressWarnings("synthetic-access")
		public AgentEventListener(BehaviorsAndInnerContextSkill_ skill) {
		...
		public void receiveEvent(Event event) {
			if (this.skill.get().running.get() && !this.skill.get().stopping.get()) {
				...
				this.skill.get().logger.warning(BehaviorsAndInnerContextSkill_.class,
