...
class SpaceRepository {
	 ...
	 * @param repositoryImplFactory - factory for creating Hazelcast data structures
	 * @param injector - injector to used for creating new spaces.
	 * @param listener - listener on the events in the space repository.
	 */
	public SpaceRepository(String distributedSpaceSetName, HazelcastDistributedDataStructureFactory repositoryImplFactory, Injector injector, SpaceRepositoryListener listener) {
	 ...
	 */
	protected synchronized void addRemoteSpace(SpaceID id) {
	 ...
	 * @param id - identifier of the space
	 */
	protected synchronized void removeRemoteSpace(SpaceID id) {
		assert(!this.spaceIDs.contains(id));
	...
	private class HazelcastListener implements ItemListener<SpaceID> {
		...
		public void itemRemoved(ItemEvent<SpaceID> item) {
			removeRemoteSpace(item.getItem());
		}
