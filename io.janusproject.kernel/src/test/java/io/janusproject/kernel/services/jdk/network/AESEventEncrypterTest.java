...
public class AESEventEncrypterTest extends Assert {
	...
	public void encrypt() throws Exception {
		...
		assertArrayEquals(
				new byte[] {76, 85, 9, -83, -86, -108, 17, 119, 91, 121, -32, 98, 19, 58, 74, -6},
				envelope.getContextId());
		
		assertArrayEquals(
				new byte[] {-108, 30, 124, -69, -57, 41, 73, -3, -85, -67, 45, -57, 36, 101, 97, 71},
				envelope.getSpaceId());

		assertArrayEquals(
				new byte[] {3, 46, -127, 55, -60, 36, -87, -85, 112, 16, -13, 55, 104, 64, 6, 104},
				envelope.getScope());

		assertArrayEquals(
				new byte[] {-15, -97, -127, 31, -113, -85, -69, 32, 33, -60, 105, -119, 71, -46, -79, 119},
				envelope.getCustomHeaders());

		assertArrayEquals(
				new byte[] {-104, -99, 112, 110, 65, 46, 68, 44, 0, -85, -69, -33, -125, -53, 100, -21},
				envelope.getBody());
	}
